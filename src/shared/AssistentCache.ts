...
export class AssistentCache {
    ...
    static findExtraLessonsInJournal(journalId: number): AssistentJournalEntry[] {
        const journal = AssistentCache.getJournal(journalId);
        if (!journal) return [];

        const extraLessons: AssistentJournalEntry[] = [];
        for (const journalEntry of journal.entriesInJournal) {
            if (!journal.entriesInTimetable.find(timetableEntry => timetableEntry.date === journalEntry.date)) {
                extraLessons.push(journalEntry);
            }
        }
        return extraLessons;
